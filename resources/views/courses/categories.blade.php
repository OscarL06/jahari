...
@section('content')
    <div class="w-full lg:w-4/5 flex flex-col items-center lg:ml-[20%]">
        ...
        <div class="w-[95%] mt-6">
            ...
            <div class="flex items-center px-4 py-5 mt-4 space-x-2 border border-gray-200 rounded-lg shadow-sm">
                ...
                @foreach($categories as $category)
                    <a href="#{{ $category->name }}" class="px-6 py-1 text-white rounded-md bg-gradient-to-r from-purple-700 to-purple-600">{{ $category->name }}</a>
                @endforeach
            </div>
        </div>
        ...
        <div class="my-6 w-[95%]">
             @foreach($categories as $category)
                <h3 id="{{ $category->name }}" class="px-3 py-0.5 mt-14 mb-3 text-3xl text-white rounded-md bg-gradient-to-r from-purple-700 to-purple-600 w-min">{{ $category->name }}</h3>
                <div class="grid grid-cols-3 gap-2">
                    @foreach($category->courses as $course)
                        <a href="/courses/{{ $course->id }}" class="relative w-11/12 p-2 border border-gray-200 rounded-md shadow">
                            ...
                            <p class="absolute text-lg text-white bottom-4 left-4">{{ $course->name }}</p>
                        </a>
                    @endforeach
                </div>
            @endforeach
